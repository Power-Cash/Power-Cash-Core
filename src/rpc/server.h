...
/**
 * PowerCash RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZprchSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzprchseed(const UniValue& params, bool fHelp);
extern UniValue getzprchseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzprch(const UniValue& params, bool fHelp);
extern UniValue dzprchstate(const UniValue& params, bool fHelp);

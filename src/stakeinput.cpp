...
// The zPRCH block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zPRCH is a hash of the serial
...
{
    //Create an output returning the zPRCH that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZPRCHOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zPRCH output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zPRCH", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZPRCHOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zPRCH output", __func__);
...
{
    CzPRCHTracker* zprchTracker = pwallet->zprchTracker.get();
    CMintMeta meta;
    if (!zprchTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zprchTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a PRCH stake is the outpoint

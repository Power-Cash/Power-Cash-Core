...
CzPRCHTracker::CzPRCHTracker(std::string strWalletFile)
{
...
CzPRCHTracker::~CzPRCHTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzPRCHTracker::Init()
{
...
bool CzPRCHTracker::Archive(CMintMeta& meta)
{
...
bool CzPRCHTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzPRCHTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzPRCHTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzPRCHTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzPRCHTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzPRCHTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzPRCHTracker::GetSerialHashes()
{
...
CAmount CzPRCHTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzPRCHTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzPRCHTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzPRCHTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzPRCHTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzPRCHTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzPRCHTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzPRCHTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzPRCHTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzPRCHTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzPRCHTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzPRCHWallet* zPRCHWallet)
{
    bool iszPRCHWalletInitialized = (NULL != zPRCHWallet);
    CMintMeta meta;
    ...
    if (! iszPRCHWalletInitialized)
        zPRCHWallet = new CzPRCHWallet(strWalletFile);
    meta.isSeedCorrect = zPRCHWallet->CheckSeed(dMint);
    if (! iszPRCHWalletInitialized)
        delete zPRCHWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzPRCHTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzPRCHTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzPRCHTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzPRCHTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzPRCHTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzPRCHTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzPRCHWallet* zPRCHWallet = new CzPRCHWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zPRCHWallet);
        }
        delete zPRCHWallet;
        LogPrint("zero", "%s: added %d dzprch from DB\n", __func__, listDeterministicDB.size());
    }

...
CzPRCHWallet::CzPRCHWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zprch seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPRCHSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPRCHSeed_deprecated()) {
                    LogPrintf("%s: Updated zPRCH seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zprch seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zprch wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzPRCHWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPRCHCount(nCountLastUsed);
    else if (!walletdb.ReadZPRCHCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPRCHWallet::Lock()
{
    seedMaster = 0;
}

void CzPRCHWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPRCHWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPRCH(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPRCHWallet::LoadMintPoolFromDB()
{
...
void CzPRCHWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPRCHWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPRCHWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zprchTracker->HasPubcoinHash(pMint.first)) {
...
bool CzPRCHWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPRCH(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zprchTracker which also adds to database
    pwalletMain->zprchTracker->Add(dMint, true);
...
void CzPRCHWallet::SeedToZPRCH(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzPRCHWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPRCHWallet::GenerateDeterministicZPRCH(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPRCHWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPRCHWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzPRCHWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());

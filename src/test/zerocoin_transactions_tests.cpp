...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zprchTracker = unique_ptr<CzPRCHTracker>(new CzPRCHTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZPRCH_TRX_FUNDS_PROBLEMS, strprintf("Failed Invalid Amount Check: %s", receipt.GetStatusMessage()));
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZPRCH_TRX_FUNDS_PROBLEMS, strprintf("Failed Invalid Amount Check: %s", receipt.GetStatusMessage()));
...
BOOST_AUTO_TEST_CASE(zerocoin_public_spend_test)
{
    ...
    CTxIn in;
    if (!ZPRCHModule::createInput(in, mint, tx.GetHash())){
        BOOST_CHECK_MESSAGE(false, "Failed to create zc input");
    }

    PublicCoinSpend publicSpend(ZCParams);
    if (!ZPRCHModule::validateInput(in, out, tx, publicSpend)){
        BOOST_CHECK_MESSAGE(false, "Failed to validate zc input");
    }

    PublicCoinSpend publicSpendTest(ZCParams);
    BOOST_CHECK_MESSAGE(ZPRCHModule::parseCoinSpend(in, tx, out, publicSpendTest), "Failed to parse public spend");
    ...
    BOOST_CHECK_MESSAGE(!ZPRCHModule::validateInput(in, out, tx, publicSpend2), "Different denomination");

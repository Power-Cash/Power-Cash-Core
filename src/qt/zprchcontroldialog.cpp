...
#include "ui_zprchcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zprchTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zprchTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zPRCH.";
            else if (!mint.isSeedCorrect)
                strReason = "The zPRCH seed used to mint this zPRCH is not the same as currently hold in the wallet";
            else

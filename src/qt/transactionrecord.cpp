...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZPRCHModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zprchTracker->HasMintTx(hash))) {
            //zPRCH stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZPRCH;
...
bool IsZPRCHType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZPRCH:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZPRCH || type == TransactionRecord::MNReward) {

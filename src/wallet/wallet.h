...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zprchspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 PRCH at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zPRCH send
enum ZerocoinSpendStatus {
    ZPRCH_SPEND_OKAY = 0,                            // No error
    ZPRCH_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZPRCH_WALLET_LOCKED = 2,                         // Wallet was locked
    ZPRCH_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZPRCH_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZPRCH_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZPRCH_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZPRCH_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZPRCH_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZPRCH_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZPRCH_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZPRCH_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZPRCH_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZPRCH_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZPRCH_SPENT_USED_ZPRCH = 14,                      // Coin has already been spend
    ZPRCH_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZPRCH_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZPRCHOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzprchAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzPRCHTracker> zprchTracker;
    ...
    void setZWallet(CzPRCHWallet* zwallet)
    {
        zwalletMain = zwallet;
        zprchTracker = std::unique_ptr<CzPRCHTracker>(new CzPRCHTracker(strWalletFile));
    }

    CzPRCHWallet* getZWallet() { return zwalletMain; }
    ...
    /** zPRCH reset */
    boost::signals2::signal<void()> NotifyzPRCHReset;

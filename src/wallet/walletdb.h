...
public:
    ...
    bool WriteZPRCHSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZPRCHSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZPRCHSeed_deprecated(uint256& seed);
    bool EraseZPRCHSeed();
    bool EraseZPRCHSeed_deprecated();

    bool WriteZPRCHCount(const uint32_t& nCount);
    bool ReadZPRCHCount(uint32_t& nCount);
